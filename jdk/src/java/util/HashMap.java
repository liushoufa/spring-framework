...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
		...
		// (n - 1) & hash 确定元素存放在哪个桶中，桶为空，新生成结点放入桶中(此时，这 个结点是放在数组中)
    ...
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
			...
			// 如果当前hash桶数组的长度在扩容后仍然小于最大容量并且oldCap大于默认值16
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)
				// double threshold 双倍扩容阀值threshold
                newThr = oldThr << 1; // double threshold
        }
		// 旧的容量为0，但threshold大于零，代表有参构造有cap传入，threshold已经被初始化成最小2的n次幂直接将该值赋给新的容量
        ...
        table = newTab;
		// 将新数组的值赋值给旧的hash桶数组，如果原先的数组没有初始化，那么resize的初始化工作到此结束，否则进入扩容元素 重排逻辑，使其均匀的分散
        if (oldTab != null) {
			// 遍历新数组的所有桶下标
                ...
                if ((e = oldTab[j]) != null) {
					...
					// e是链表的头并且e.next!=null，那么处理链表中元素重排
                        ...
                        do {
                            ...
                            if ((e.hash & oldCap) == 0) {
                                if (loTail == null)
									// 初始化head指向链表当前元素e，e不一定是链表的第一个元素，初始化后loHead代表下标保持不变的链表的头元素
                                    loHead = e;
                                else
									// loTail指向当前的元素e初始化后，loTail和loHead指向相同的内存，所以当loTail.next指向下一个元素时
									// 底层数组中的元素的next引用也相应发生变化，造成lowHead.next.next..... 71
